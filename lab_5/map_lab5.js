...
var greatwall = L.marker(coords[0], {icon: myIcon1}).bindPopup(greatwallPopup, customOptions).addTo(landmarks);

var colo = L.marker(coords[1], {icon: myIcon2}).bindPopup(coloPopup, customOptions).addTo(landmarks);

var petra = L.marker(coords[2], {icon: myIcon3}).bindPopup(petraPopup, customOptions).addTo(landmarks);

var chichen = L.marker(coords[3], {icon: myIcon4}).bindPopup(chichenPopup, customOptions).addTo(landmarks);

var machu = L.marker(coords[4], {icon: myIcon5}).bindPopup(machuPopup, customOptions).addTo(landmarks);

var christ = L.marker(coords[5], {icon: myIcon6}).bindPopup(christPopup, customOptions).addTo(landmarks);

var taj = L.marker(coords[6], {icon: myIcon7}).bindPopup(tajPopup, customOptions).addTo(landmarks);
...
var overlays = {
    '7 Wonders of the World': landmarks,
    'Travel Path': line
};
...
//Pop-up for showing coordinates
//Create empty popup
var popup = L.popup();

//Function for pop up contents
function onMapClick(e) {
    popup
        .setLatLng(e.latlng)
        .setContent(
    "You clicked the map at -<br>" + "<b>long:</b> " + e.latlng.lng + "<br>" + "<b>long:</b> " + e.latlng.lat).openOn(mymap);
}
...
//Update the Lat/Long based new reading
async function getISS() {
    var response = await fetch(api_url);
    var data = await response.json();
    var { latitude, longitude } = data;
    
//Change the marker location but keep map view on center
